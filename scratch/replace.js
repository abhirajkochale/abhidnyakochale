const fs = require('fs');
const code = fs.readFileSync('./src/pages/PoemsPage.jsx', 'utf8');

const newPoetryBook = `function PoetryBook() {
  return (
    <>
      <style>
        {\`
...
const startIndex = code.indexOf('function PoetryBook() {');
const endIndex = code.indexOf('export default function PoemsPage() {');

if (startIndex > -1 && endIndex > -1) {
  const newCode = code.substring(0, startIndex) + newPoetryBook + '\n' + code.substring(endIndex);
  fs.writeFileSync('./src/pages/PoemsPage.jsx', newCode);
  console.log('PoetryBook Replaced successfully!');
} else {
  console.log('Could not find component bounds in PoemsPage.jsx');
}
